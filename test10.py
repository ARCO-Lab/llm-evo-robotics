...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=8.0):
    ...
    for i in range(gene['num_links']):
        ...
        p.createConstraint(parent_body, -1, child_body, -1, jt,
        ...
        pos_parent = p.getBasePositionAndOrientation(parent_body)[0]
        pos_child = p.getBasePositionAndOrientation(child_body)[0]
        mid_point = [(a + b) / 2 for a, b in zip(pos_parent, pos_child)]
