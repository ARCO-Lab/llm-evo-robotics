# evolve_robot_2_0_full_fixed.py - 修复后的完整版本，避免使用 JOINT_FIXED 作为 link 关节
import numpy as np
import random
...
def decode_gene(x, max_links=6):
    ...
    for i in range(num_links):
        # 仅允许 REVOLUTE 和 PRISMATIC，避免 FIXED
...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=8.0):
    ...
    if num_links == 0:
        ...
        linkParentIndices = [0] * num_links  # 所有link接在base上更稳
        linkJointTypes = []
        linkJointAxis = []
        ...
        for i in range(num_links):
            ...
            jt = gene['joint_types'][i + 1]
            linkJointTypes.append(jt)
