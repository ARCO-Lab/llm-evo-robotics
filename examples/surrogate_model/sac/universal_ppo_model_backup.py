...
class UniversalPPOCritic(nn.Module):
    ...
    def __init__(self, attn_model, hidden_dim=256, device='cpu'):
        ...
        self.attn_model = attn_model  # 🆕 直接使用传入的独立attention模型
        self.device = device
        ...
        self.value_scale = 10.0  # 🔧 修复：匹配归一化后的奖励范围
        
    def forward(self, joint_q, vertex_k, vertex_v, vertex_mask=None):
        """前向传播，输出状态价值"""
        batch_size, num_joints, feature_dim = joint_q.shape
        
        # 🆕 首先使用主要attention模型处理输入
        main_attn_output = self.attn_model(joint_q, vertex_k, vertex_v, vertex_mask)  # [B, num_joints]
        
        # 投影原始输入到128维
        joint_q_proj = self.input_projection(joint_q)  # [B, num_joints, 128]
        
        # 🆕 将主要attention输出作为权重调制原始特征
        attn_weights = torch.sigmoid(main_attn_output).unsqueeze(-1)  # [B, num_joints, 1]
        combined_features = joint_q_proj * attn_weights  # [B, num_joints, 128] - 注意力加权
        
        # 🎯 使用内部value attention进行最终聚合
        # 创建全局查询
        global_query = combined_features.mean(dim=1, keepdim=True)  # [B, 1, 128]
        
        # 注意力聚合
        attn_output, _ = self.value_attention(
            global_query,    # query: [B, 1, 128]
            combined_features,    # key: [B, num_joints, 128] 
            combined_features     # value: [B, num_joints, 128]
        )  # output: [B, 1, 128]
        ...
        # 🆕 添加：额外的输出限制，防止极端值导致loss爆炸
        value = torch.clamp(value, -20.0, 20.0)
...
class UniversalPPOWithBuffer:
    ...
    def __init__(self, buffer_size=2048, batch_size=64, lr=1e-4, gamma=0.99, 
                 ...
                 value_coef=0.5, max_grad_norm=2.0, device='cpu', env_type='reacher2d'):
        ...
        # 🆕 创建完全独立的attention网络（更清晰的架构）
        self.actor_attn = UniversalAttnModel(128, 130, 130, 4)
        self.critic_attn = UniversalAttnModel(128, 130, 130, 4)
        self.actor = UniversalPPOActor(self.actor_attn, device=device)
        self.critic = UniversalPPOCritic(self.critic_attn, device=device)
        ...
        # 🆕 Attention梯度存储（修复梯度计算时机问题）
        self._last_actor_attn_grad = 0.0
        self._last_critic_main_attn_grad = 0.0      # Critic主要attention
        self._last_critic_value_attn_grad = 0.0     # Critic内部value attention
    ...
    def update(self, next_obs=None, next_gnn_embeds=None, num_joints=None, ppo_epochs=4):
        ...
        # 🆕 添加attention梯度累积变量（修复时机问题）
        total_actor_attn_grad = 0.0
        total_critic_main_attn_grad = 0.0      # Critic主要attention
        total_critic_value_attn_grad = 0.0     # Critic内部value attention
        attention_update_count = 0
        ...
        for epoch in range(ppo_epochs):
            ...
            for num_joints, experiences in joint_groups.items():
                ...
                # 🆕 在梯度裁剪之前计算Actor attention梯度（修复时机问题）
                ...
                if actor_attn_param_count > 0:
                    ...
                    total_actor_attn_grad += actor_attn_grad_norm
                ...
                # 🆕 在step()之前分别计算三个attention网络的梯度
                # 1. Critic主要attention模型梯度
                critic_main_attn_grad_norm = 0.0
                critic_main_attn_param_count = 0
                for name, param in self.critic.attn_model.named_parameters():
                    if param.grad is not None:
                        param_norm = param.grad.data.norm(2).item()
                        critic_main_attn_grad_norm += param_norm ** 2
                        critic_main_attn_param_count += 1
                if critic_main_attn_param_count > 0:
                    critic_main_attn_grad_norm = (critic_main_attn_grad_norm ** 0.5) / critic_main_attn_param_count
                    total_critic_main_attn_grad += critic_main_attn_grad_norm
                
                # 2. Critic内部value_attention梯度
                critic_value_attn_grad_norm = 0.0
                critic_value_attn_param_count = 0
                for name, param in self.critic.value_attention.named_parameters():
                    if param.grad is not None:
                        param_norm = param.grad.data.norm(2).item()
                        critic_value_attn_grad_norm += param_norm ** 2
                        critic_value_attn_param_count += 1
                if critic_value_attn_param_count > 0:
                    critic_value_attn_grad_norm = (critic_value_attn_grad_norm ** 0.5) / critic_value_attn_param_count
                    total_critic_value_attn_grad += critic_value_attn_grad_norm
                
                attention_update_count += 1
        ...
        # 🆕 在清空buffer之前记录buffer大小
        buffer_size_before_clear = len(self.buffer.experiences)
        
        # 🆕 保存计算的attention梯度（修复时机问题）
        if attention_update_count > 0:
            self._last_actor_attn_grad = total_actor_attn_grad / attention_update_count
            self._last_critic_main_attn_grad = total_critic_main_attn_grad / attention_update_count
            self._last_critic_value_attn_grad = total_critic_value_attn_grad / attention_update_count
        else:
            self._last_actor_attn_grad = 0.0
            self._last_critic_main_attn_grad = 0.0
            self._last_critic_value_attn_grad = 0.0
        ...
        metrics = {
            ...
            'buffer_size': buffer_size_before_clear,  # 🆕 使用清空前的buffer大小
        ...
        if metrics['critic_loss'] > 10.0:  # 🔧 修复：提高阈值，避免过度干预
            if not SILENT_MODE:
                print(f"🚨 Critic loss异常高 {metrics['critic_loss']:.2f}，温和调整学习率")
            for param_group in self.critic_optimizer.param_groups:
                param_group['lr'] *= 0.8  # 🔧 修复：更温和的衰减
                # 🆕 设置学习率下限，防止过度衰减
    ...
    def _calculate_attention_losses_and_focus(self):
        ...
        try:
            # 🆕 使用预计算的梯度值（修复时机问题）
            actor_attn_grad_norm = getattr(self, '_last_actor_attn_grad', 0.0)
            critic_main_attn_grad_norm = getattr(self, '_last_critic_main_attn_grad', 0.0)
            critic_value_attn_grad_norm = getattr(self, '_last_critic_value_attn_grad', 0.0)
            
            # 🆕 独立记录三个attention网络的梯度
            attention_metrics['attention_actor_grad_norm'] = actor_attn_grad_norm
            attention_metrics['attention_critic_main_grad_norm'] = critic_main_attn_grad_norm
            attention_metrics['attention_critic_value_grad_norm'] = critic_value_attn_grad_norm
            
            # 🆕 分别计算各自的loss
            attention_metrics['attention_actor_loss'] = actor_attn_grad_norm
            attention_metrics['attention_critic_main_loss'] = critic_main_attn_grad_norm
            attention_metrics['attention_critic_value_loss'] = critic_value_attn_grad_norm
            attention_metrics['attention_total_loss'] = actor_attn_grad_norm + critic_main_attn_grad_norm + critic_value_attn_grad_norm
            
            # 4. 🆕 分别计算Actor和Critic的attention网络参数统计
            # 4.1 Actor attention网络参数 (完全独立的网络)
            ...
            if actor_attn_params:
                ...
                attention_metrics['attention_actor_param_count'] = len(actor_attn_params)
            
            # 4.2 Critic attention网络参数 (完全独立的网络)
            critic_attn_params = list(self.critic.attn_model.parameters())
            if critic_attn_params:
                ...
                attention_metrics['attention_critic_param_count'] = len(critic_attn_params)
