...
import sys
import os
sb3_path = os.path.join(os.path.dirname(__file__), "sac")
...
def main(args):
    ...
    # 🔧 设置 alpha (熵系数)
    if USE_SB3_SAC:
        sac.alpha = args.alpha  # SB3 SAC 使用属性设置
    else:
        sac.alpha = torch.tensor(args.alpha)  # 原始 SAC 使用 tensor
    sac.min_alpha = 0.05
...
#     # SB3 或原始 SAC 创建
#     if USE_SB3_SAC:
#         sac = SB3SACFactory.create_reacher_sac(action_dim=num_joints, buffer_capacity=10000, batch_size=64, lr=1e-5, device='cpu')
#         sac.set_env(env)
#     else:
#         sac = AttentionSACWithBuffer(attn_model, num_joints, buffer_capacity=10000, batch_size=64, lr=1e-5, env_type='reacher2d')
