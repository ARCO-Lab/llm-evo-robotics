...
joint_q_input = prepare_joint_q_input(test_obs, gnn_embeds_test)

vertex_k_test = gnn_embeds_test
joint_vel = test_obs[:, 28:40]
node_velocities = torch.cat([joint_vel.unsqueeze(-1), gnn_embeds_test], dim=-1)
vertex_v_test = torch.cat([gnn_embeds_test, node_velocities], dim=-1)

print(joint_q_input.shape)