...
class Reacher2DEnv(Env):
    ...
    def __init__(self, num_links=2, link_lengths=None, render_mode=None, config_path=None, curriculum_stage=1, debug_level='SILENT'):
        super().__init__()
            ...
            # 🆕 维持奖励系统
        # 🔧 调整维持参数 - 让机器人更容易开始维持
        self.maintain_threshold = 100.0  # 维持距离阈值（从20px扩大到100px）
        self.maintain_target_steps = 150  # 目标维持步数（从500步减少到150步）
    ...
    def _compute_reward(self, collision_detected=None):
        ...
        if distance <= 50.0:
            # 非常接近：巨大的正奖励
            distance_reward = 10.0 - distance / 5.0  # 范围: [0, 10]
        elif distance <= 100.0:
            # 较近：中等正奖励  
            distance_reward = 5.0 - distance / 10.0   # 范围: [-5, 5]
        elif distance <= 200.0:
            # 中等距离：小幅负奖励
            distance_reward = -distance / 50.0        # 范围: [-4, -2]
        else:
            # 远距离：较大负奖励
            distance_reward = -distance / 25.0        # 范围: [-∞, -8]
        ...
        if distance < self.maintain_threshold:
            ...
            if self.maintain_counter <= self.maintain_target_steps:
                ...
                maintain_reward = 10.0 * maintain_progress  # 从5.0增加到10.0，更强激励
                
                # 🎉 更频繁的里程碑奖励 - 针对新的150步目标
                milestone_intervals = [10, 25, 50, 75, 100, 125, 150]  # 适应150步目标
                if self.maintain_counter in milestone_intervals:
                    if self.maintain_counter <= 50:
                        milestone_bonus = 15.0  # 早期里程碑给更大奖励
                    elif self.maintain_counter <= 100:
                        milestone_bonus = 20.0  # 中期里程碑
                    else:
                        milestone_bonus = 30.0  # 后期里程碑给巨大奖励
                    maintain_reward += milestone_bonus
                    print(f"🏆 维持里程碑! {self.maintain_counter}步 (+{milestone_bonus:.1f})")
            ...
            if self.in_maintain_zone:
                # 🔧 减少离开维持区域的惩罚，鼓励重新尝试
                leave_penalty = -5.0  # 从-20.0减少到-5.0，温和惩罚
        ...
        progress = self.prev_distance - distance  # 正值表示靠近目标
        
        # 根据距离调整进步奖励权重
        if distance <= 100.0:
            progress_weight = 20.0  # 接近目标时，进步非常重要
        elif distance <= 200.0:
            progress_weight = 10.0  # 中等距离时，进步重要
        else:
            progress_weight = 5.0   # 远距离时，基础权重
        
        progress_reward = np.clip(progress * progress_weight, -5.0, 5.0)
        
        # 到达奖励（保持原有逻辑但调整）
        if distance < 20.0:
            reach_reward = 20.0  # 大幅增加到达奖励
        elif distance < 50.0:
            reach_reward = 10.0  # 接近时也给奖励
        else:
            reach_reward = 0.0
        ...
        self.reward_components = {
            ...
            'progress_reward': progress_reward,  # 🆕 进步奖励
        ...
        # 更新prev_distance (在计算progress_reward之后)
        self.prev_distance = distance
        
        # 🔧 新的总奖励计算 - 包含所有组件
        total_reward = (distance_reward + reach_reward + progress_reward + maintain_reward + 
