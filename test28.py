# evolve_robot_2_0_full_safe.py - 加强版：修复 orientation 引用、增加调试与断言
import numpy as np
import random
...
def decode_gene(x, max_links=6):
    ...
    for i in range(num_links):
        jt_val = int(x[idx + 1] * 2)
        jt = [p.JOINT_REVOLUTE, p.JOINT_PRISMATIC][jt_val]
...
def create_link_shape(shape_id, is_wheel=False):
    ...
    assert isinstance(col, int) and col >= 0, "Invalid collision shape"
    assert isinstance(vis, int) and vis >= 0, "Invalid visual shape"
    return col, vis
...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=8.0):
    ...
    if num_links == 0:
        ...
        linkPositions = []
        linkOrientations = [[0, 0, 0, 1] for _ in range(num_links)]  # 独立对象
        ...
        print("[DEBUG] linkMasses =", linkMasses)
        print("[DEBUG] linkJointTypes =", linkJointTypes)
        print("[DEBUG] linkCollisionShapeIndices =", linkCollisionShapeIndices)
        print("[DEBUG] linkVisualShapeIndices =", linkVisualShapeIndices)
        print("[DEBUG] linkPositions =", linkPositions)
        print("[DEBUG] linkOrientations =", linkOrientations)
        print("[DEBUG] linkParentIndices =", linkParentIndices)
        print("[DEBUG] linkJointAxis =", linkJointAxis)

        assert len(linkMasses) == len(linkCollisionShapeIndices) == len(linkVisualShapeIndices) == \
               len(linkPositions) == len(linkOrientations) == len(linkParentIndices) == \
               len(linkJointTypes) == len(linkJointAxis), "All link arrays must be the same length!"
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for idx, x in enumerate(X):
            ...
            try:
                ...
                print(f"[ERROR] Evaluation {idx} failed: {e}")
                fitness = 0.0
