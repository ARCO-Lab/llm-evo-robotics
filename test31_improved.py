...
def decode_gene(x, max_links=8):
    # 修改链接数计算方式，确保能生成更多链接
    # 原来的计算方式: num_links = max(2, int(x[0] * max_links) + 1)
    # 这会导致x[0]很小时总是只有2个链接
    num_links = max(3, int(x[0] * max_links) + 1)
    ...
    for i in range(num_links):
        ...
        # 是否为轮子
        wheel = shape == 1 and x[idx + 3] < 0.4  # 圆柱体有40%概率成为轮子
...
def generate_urdf(gene):
    ...
    <inertial>
      ...
      <inertia ixx="0.0054" ixy="0" ixz="0" iyy="0.0054" iyz="0" izz="0.0108"/>
        ...
        # 根据形状和索引调整关节位置，使结构更合理
        if shape_id == 1 and is_wheel:  # 轮子应该在侧面
            # 轮子交替放在左右两侧，并根据索引调整前后位置
            side = 0.15 if i % 2 == 0 else -0.15
            front = 0.1 * (i % 3)  # 根据索引调整前后位置
            urdf += f'''  <joint name="joint{i}" type="{joint_type}">
    <parent link="{parent}"/>
    <child link="link{i}"/>
    <origin xyz="{front} {side} 0" rpy="0 0 0"/>
    <axis xyz="{axis_str}"/>
    <limit lower="-3.14" upper="3.14" effort="30" velocity="20"/>
  </joint>\n'''
        elif shape_id == 4:  # 大轮子
            # 大轮子交替放在左右两侧
            side = 0.2 if i % 2 == 0 else -0.2
            front = 0.1 * (i % 3)  # 根据索引调整前后位置
            urdf += f'''  <joint name="joint{i}" type="{joint_type}">
    <parent link="{parent}"/>
    <child link="link{i}"/>
    ...
    <limit lower="-3.14" upper="3.14" effort="50" velocity="15"/>
            ...
            # 根据链接索引分配不同位置
            if i % 4 == 0:
                pos = f"{0.15 + (i % 3) * 0.05} 0 0"  # 前方
            elif i % 4 == 1:
                pos = f"0 {0.12 + (i % 3) * 0.03} 0"  # 右侧
            elif i % 4 == 2:
                pos = f"0 {-0.12 - (i % 3) * 0.03} 0"  # 左侧
            else:
                pos = f"0 0 {0.08 + (i % 3) * 0.04}"  # 上方
...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=20.0, use_self_collision=True):
    ...
    # 增加对链接数量和结构复杂性的奖励
    complexity_bonus = min(2.0, (gene['num_links'] - 2) * 0.3)  # 每增加一个链接增加0.3的奖励
    wheel_bonus = sum(gene['is_wheel']) * 0.2  # 每个轮子增加0.2的奖励
    motor_bonus = sum(gene['has_motor']) * 0.1  # 每个马达增加0.1的奖励
    
    fitness = (1.0 / (1e-3 + dist_to_goal)) + base_move + path_linearity + stability_score + complexity_bonus + wheel_bonus + motor_bonus
...
class CustomInitialSampling(Sampling):
    ...
    def _do(self, problem, n_samples, **kwargs):
        ...
        for i in range(n_samples // 3):
            # 设置第一个基因值较大，确保链接数较多
            X[i, 0] = 0.7 + 0.3 * np.random.random()
            
        # 确保另外1/3的个体有中等链接数
        for i in range(n_samples // 3, 2 * n_samples // 3):
            X[i, 0] = 0.4 + 0.3 * np.random.random()
...
class RobotMoveToTargetProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for idx, x in enumerate(X):
            ...
            joint_types = [
                "旋转" if jt == p.JOINT_REVOLUTE else 
                "固定" if jt == p.JOINT_FIXED else 
                "伸缩" if jt == p.JOINT_PRISMATIC else "连续" 
                for jt in gene['joint_types']
            ]
            
            print(f"[评估 {idx}] 链接数={gene['num_links']} 马达数={motor_count} 轮子数={wheel_count} 关节类型={joint_types} 适应度={fitness:.4f}")
...
if __name__ == "__main__":
    ...
    algorithm = NSGA2(
        pop_size=50,  # 增加种群大小
        crossover=SBX(prob=0.9, eta=12),  # 降低eta增加交叉变异幅度
        mutation=PM(prob=0.3, eta=12),  # 大幅增加变异概率和降低eta以提高变异幅度
        eliminate_duplicates=True,
        sampling=CustomInitialSampling()  # 使用自定义采样
    )

    res = minimize(problem,
                   algorithm,
                   termination=('n_gen', 50),  # 增加进化代数
                   seed=42,  # 使用不同的随机种子
                   verbose=True,
                   save_history=True)
