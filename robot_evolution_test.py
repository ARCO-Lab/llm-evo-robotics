...
def decode_gene(x, config=None, adaptation_state=None):
    ...
    default_config = {
        ...
        'link_params': 15,        # 每个额外连杆的参数数量（增加了2个父节点和连接位置参数）
        'joint_type_ranges': [0.3, 0.7],  # 修改为只有两个阈值：0.3以下为REVOLUTE，0.3-0.7为PRISMATIC，0.7以上为FIXED
        ...
        'max_branch_depth': 4,                  # 最大分支深度
        'branching_probability': 0.4,           # 分支概率
    }
    ...
    parent_indices = []  # 新增：父节点索引
    connection_points = []  # 新增：连接点位置 (0-1之间的相对位置)
    node_depths = []  # 新增：节点深度
    ...
    parent_indices.append(-1)  # 底座没有父节点
    connection_points.append(0.5)  # 默认连接点位置
    node_depths.append(0)  # 底座深度为0
    ...
    # 3. 解码其余连杆 - 支持树形结构
    # 创建每层节点的索引列表 - 用于树形结构生成
    level_nodes = [[0]]  # 第0层只有底座
    current_level = 1
    
    # 对于每个节点(从1开始，跳过底座)，决定其父节点和深度
    for i in range(1, num_links):
        ...
        if gene_start + config['link_params'] > gene_length:
            ...
            # 如果已经有其他节点，考虑连接到更深层次的节点
            if i > 1 and len(level_nodes) > 1:
                prev_level = max(0, min(current_level - 1, len(level_nodes) - 1))
                if level_nodes[prev_level]:  # 如果上一层有节点
                    default_parent = np.random.choice(level_nodes[prev_level])
                    default_depth = node_depths[default_parent] + 1
            
            parent_indices.append(default_parent)
            connection_points.append(0.5)  # 默认连接点位置
            node_depths.append(default_depth)
            
            # 将此节点添加到相应深度的列表中
            while len(level_nodes) <= default_depth:
                level_nodes.append([])
            level_nodes[default_depth].append(i)
        ...
        # 收集所有可能的父节点 - 仅考虑较低深度的节点
        for level in range(min(len(level_nodes), max_branch_depth)):
            potential_parents.extend(level_nodes[level])
        ...
        # 根据基因值选择父节点
        parent_idx = potential_parents[int(parent_gene * len(potential_parents)) % len(potential_parents)]
        
        # 确定节点深度
        node_depth = node_depths[parent_idx] + 1
        ...
        # 将此节点添加到相应深度的列表中
        while len(level_nodes) <= node_depth:
            level_nodes.append([])
        level_nodes[node_depth].append(i)
        
        # 连接点位置 - 沿父节点主轴的相对位置(0-1)
        if gene_start < gene_length:
            connection_point = x[gene_start]
            connection_points.append(connection_point)
            gene_start += 1
        else:
            connection_points.append(0.5)  # 默认中点
        ...
        if gene_start < gene_length:
            ...
            # 叶子节点(没有子节点的节点)更可能是轮子
            is_leaf = True
            for j in range(i+1, num_links):
                if j < len(parent_indices) and parent_indices[j] == i:
                    is_leaf = False
                    break
            
            # 如果是叶子节点，增加轮子概率
            if is_leaf:
                is_wheel_val = is_wheel_val or (x[gene_start] > 0.5)
    ...
    return {
        ...
        'parent_indices': parent_indices,       # 新增：父节点索引
        'connection_points': connection_points, # 新增：连接点位置
        'node_depths': node_depths             # 新增：节点深度
    }
...
def generate_urdf(gene):
  ...
  <material name="wheel_material">
    ...
    # 获取树形结构参数
    parent_indices = gene.get('parent_indices', [])
    connection_points = gene.get('connection_points', [])
    node_depths = gene.get('node_depths', [])
    
    # 如果未提供树形结构参数，创建默认值（所有连杆连接到底座）
    if not parent_indices or len(parent_indices) < num_links:
        parent_indices = [-1]  # 底座没有父节点
        parent_indices.extend([0] * (num_links - 1))  # 其他连杆连接到底座
    
    if not connection_points or len(connection_points) < num_links:
        connection_points = [0.5] * num_links
    
    if not node_depths or len(node_depths) < num_links:
        node_depths = [0]  # 底座深度为0
        node_depths.extend([1] * (num_links - 1))  # 其他连杆深度为1
    ...
    if num_links > 1:
        ...
        for i in range(1, num_links):
            ...
            parent_name = "base_link" if parent_idx == 0 else f"link{parent_idx}"
    ...
    <inertial>
      ...
      <inertia ixx="{2*radius**2/5}" ixy="0" ixz="0" 
            ...
            if parent_idx >= 0 and parent_idx < len(link_sizes):
                parent_size = link_sizes[parent_idx]
                child_size = link_sizes[i]
                
                # 获取父节点形状
                parent_shape = shapes[parent_idx]
                
                # 根据父节点形状和连接点确定连接位置
                if parent_shape == 0:  # 盒子
                    # 使用连接点决定连接位置
                    connection_point = connection_points[i]
                    
                    # 确定连接面
                    face = int(connection_point * 6)  # 0-5: 右左前后上下
                    connection_point_on_face = connection_point * 6 - face  # 面上的相对位置(0-1)
                    
                    if face == 0:  # 右面 (+X)
                        pos_x = parent_size[0] / 2
                        pos_y = parent_size[1] * (connection_point_on_face * 2 - 1)
                        pos_z = 0
                    elif face == 1:  # 左面 (-X)
                        pos_x = -parent_size[0] / 2
                        pos_y = parent_size[1] * (connection_point_on_face * 2 - 1)
                        ...
                        pos_x = parent_size[0] * (connection_point_on_face * 2 - 1)
                        pos_y = parent_size[1] / 2
                        pos_z = 0
                    elif face == 3:  # 后面 (-Y)
                        pos_x = parent_size[0] * (connection_point_on_face * 2 - 1)
                        pos_y = -parent_size[1] / 2
                        ...
                        pos_x = parent_size[0] * (connection_point_on_face * 2 - 1)
                        pos_y = 0
                        pos_z = parent_size[2] / 2
                    else:  # 下面 (-Z)
                        pos_x = parent_size[0] * (connection_point_on_face * 2 - 1)
                        pos_y = 0
                        pos_z = -parent_size[2] / 2
                    
                elif parent_shape == 1:  # 圆柱
                    # 对于圆柱体，使用角度来确定连接位置
                    parent_radius = (parent_size[0] + parent_size[1]) / 4
                    parent_height = parent_size[2]
                    
                    # 计算连接点角度和高度
                    angle = connection_points[i] * 2 * np.pi
                    height_ratio = connection_points[i] * 2 - 1  # -1 到 1
                    
                    # 连接在侧面还是顶面/底面
                    if abs(height_ratio) > 0.8:  # 连接在顶面或底面
                        pos_x = 0
                        pos_y = 0
                        pos_z = np.sign(height_ratio) * parent_height / 2
                    else:  # 连接在侧面
                        pos_x = parent_radius * np.cos(angle)
                        pos_y = parent_radius * np.sin(angle)
                        pos_z = height_ratio * parent_height / 2
                
                else:  # 球体
                    # 对于球体，使用球坐标系确定连接位置
                    parent_radius = min(parent_size) / 2
                    
                    # 计算球坐标系参数
                    phi = connection_points[i] * np.pi  # 天顶角(0-π)
                    theta = connection_points[i] * 2 * np.pi  # 方位角(0-2π)
                    
                    # 计算笛卡尔坐标
                    pos_x = parent_radius * np.sin(phi) * np.cos(theta)
                    pos_y = parent_radius * np.sin(phi) * np.sin(theta)
                    pos_z = parent_radius * np.cos(phi)
                ...
                if is_wheel[i]:
                    ...
                    # 轮子半径和厚度
                    wheel_radius = child_size[0]
                    wheel_width = child_size[2]
                    
                    # 放置轮子，使其略微超出底盘
                    pos_x += direction[0] * (wheel_radius + offset)
                    pos_y += direction[1] * (wheel_radius + offset)
                    pos_z += direction[2] * (wheel_radius + offset)
                    
                    # 轮子的方向 - 垂直于连接方向
                    if abs(direction[2]) > 0.9:  # 大致垂直方向
                        rpy = "1.5708 0 0"  # 旋转90度，使轮子垂直于地面
                    else:
                        # 计算轮子应该旋转的角度，使其可以滚动
                        roll = 1.5708  # 默认90度
                    ...
                    # 非轮子连接 - 简单偏移
                    pos_x += 0 if abs(pos_x) < 0.001 else np.sign(pos_x) * offset
                    pos_y += 0 if abs(pos_y) < 0.001 else np.sign(pos_y) * offset
                    pos_z += 0 if abs(pos_z) < 0.001 else np.sign(pos_z) * offset
    ...
    <origin xyz="{pos_x} {pos_y} {pos_z}" rpy="{rpy}"/>
...
def update_adaptation_state(adaptation_state, evaluated_designs, current_generation):
    ...
    if adaptation_state is None:
        adaptation_state = {
            'max_links': 8,                 # 默认最大连杆数
            'joint_type_ranges': [0.3, 0.7],  # 默认关节类型阈值 - REVOLUTE, PRISMATIC, FIXED
    ...
    for design in top_designs:
        ...
        # 收集树结构信息
        if 'node_depths' in config:
            max_depths.append(max(config['node_depths']))
        
        # 计算分支因子 - 父节点的平均子节点数
        if 'parent_indices' in config:
            parent_counts = {}
            for parent_idx in config['parent_indices'][1:]:  # 跳过底座
    ...
    # 更新关节类型分布 - 只考虑REVOLUTE, PRISMATIC和FIXED
    total_joints = joint_type_counts[p.JOINT_REVOLUTE] + joint_type_counts[p.JOINT_PRISMATIC] + joint_type_counts[p.JOINT_FIXED]
    if total_joints > 0:
        # 计算REVOLUTE的比例
        revolute_ratio = joint_type_counts[p.JOINT_REVOLUTE] / total_joints
        # 计算PRISMATIC的比例
        prismatic_ratio = joint_type_counts[p.JOINT_PRISMATIC] / total_joints
        
        # 设置阈值 - 确保REVOLUTE, PRISMATIC和FIXED都有机会
        threshold1 = max(0.2, min(0.5, revolute_ratio))
        threshold2 = max(0.6, min(0.9, threshold1 + prismatic_ratio))
        
        adaptation_state['joint_type_ranges'] = [threshold1, threshold2]
    
    # 更新树结构参数
    if max_depths:
        avg_max_depth = sum(max_depths) / len(max_depths)
        # 根据表现优秀的设计，调整最大分支深度
        adaptation_state['max_branch_depth'] = max(3, min(6, int(avg_max_depth + 0.5)))
    
    if branching_factors:
        avg_branching = sum(branching_factors) / len(branching_factors)
        # 调整分支概率
        branching_prob = min(0.7, max(0.3, avg_branching / 3.0))
        adaptation_state['branching_probability'] = branching_prob
...
def simulate_robot_multi(gene, config=None):
    ...
    default_config = {
        ...
        'terrain_type': 'flat',
        'goal_pos': [5.0, 0, 0.1],
        'apply_initial_control': False,  # 是否应用初始电机力
        'control_delay': 1.0,            # 延迟多久后应用控制（秒）
        'terrain_params': {
            'size': 256,
            'scale': [0.05, 0.05, 0.2],
            'height_range': [0, 0.4]
...
def load_robot_from_gene(gene, use_self_collision):
    ...
    robot_id = p.loadURDF(urdf_path, basePosition=[0, 0, 0.5], flags=flags)
    
    return robot_id, urdf_path

def setup_robot_joints(robot_id, gene, config):
    """设置机器人关节控制并返回关节数据，但不应用初始动力"""
    joint_indices = []
    x_axis_wheels = []
    y_axis_wheels = []
    z_axis_wheels = []
    ...
    # 检查是否应用初始控制
    apply_initial_control = config.get('apply_initial_control', False)
    # 获取一个很小的力值，足以防止漂移但不足以产生显著运动
    min_force = 0.01  # 非常小的力
    ...
    for i in range(p.getNumJoints(robot_id)):
        ...
        if "wheel" in joint_name.lower():
            is_wheel = True
        # 2. 通过基因参数判断
        elif 'is_wheel' in gene and i < len(gene['is_wheel']) and gene['is_wheel'][i]:
            is_wheel = True
        ...
        if is_wheel:
            ...
            if max_axis_idx == 0:  # X轴
                ...
                p.setJointMotorControl2(
                    ...
                    targetVelocity=0.0, 
                    force=min_force if not apply_initial_control else config['wheel_control']['force']
                )
            ...
            p.setJointMotorControl2(
                ...
                targetPosition=0, 
                force=min_force if not apply_initial_control else config['joint_control']['force']
            )
    
    if config['gui']:
        print(f"Found wheel joints: X-axis={len(x_axis_wheels)}, Y-axis={len(y_axis_wheels)}, Z-axis={len(z_axis_wheels)}")
...
def run_simulation(robot_id, goal_pos, joint_data, config):
    ...
    # 计算控制延迟步数
    control_delay_steps = int(config.get('control_delay', 0) / dt)
    
    # 仿真循环
    for step in range(steps):
        # 如果已经过了延迟时间，应用正常控制力
        if step >= control_delay_steps:
            # 应用轮子控制
            for i in joint_data['x_axis_wheels']:
                p.setJointMotorControl2(
                    robot_id, i, p.VELOCITY_CONTROL, 
            ...
            for i in joint_data['y_axis_wheels']:
                p.setJointMotorControl2(
                    robot_id, i, p.VELOCITY_CONTROL, 
                    targetVelocity=-config['wheel_control']['velocity'], 
                    force=config['wheel_control']['force']
            ...
            for i in joint_data['z_axis_wheels']:
                p.setJointMotorControl2(
                    robot_id, i, p.VELOCITY_CONTROL, 
                    targetVelocity=config['wheel_control']['velocity'], 
                    force=config['wheel_control']['force']
...
def visualize_robot(urdf_file, sim_time=10.0, terrain_type="flat"):
    ...
    # 加载机器人
    robot_id = p.loadURDF(urdf_file, basePosition=[0, 0, 0.5])
