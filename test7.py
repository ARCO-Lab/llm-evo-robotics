...
def simulate_robot(gene, gui=False, show_goal=True):
    ...
    for i in range(gene['num_links']):
        ...
        if motor and jt == p.JOINT_REVOLUTE:
            p.setJointMotorControl2(child_body, 0, p.VELOCITY_CONTROL, targetVelocity=5.0, force=100.0)
    ...
    distance = np.linalg.norm(np.array(base_pos) - np.array(goal_pos))
    p.disconnect()
