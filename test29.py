# evolve_robot_2_0_debug.py - 完整修复并加断言校验的版本
...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=8.0):
    ...
    if num_links == 0:
        ...
        linkParentIndices = [0 for _ in range(num_links)]
        linkJointTypes = []
        linkJointAxis = []
        ...
        # Debug: 验证数组长度是否一致
        arrays = [linkMasses, linkCollisionShapeIndices, linkVisualShapeIndices, linkPositions,
                  linkOrientations, linkParentIndices, linkJointTypes, linkJointAxis]
        names = ["masses", "collision", "visual", "pos", "orient", "parent", "jointType", "axis"]
        lengths = [len(a) for a in arrays]
        assert len(set(lengths)) == 1, f"❌ createMultiBody 参数长度不一致: {dict(zip(names, lengths))}"
