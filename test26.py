# evolve_robot_2_0_full_fixed.py - 修复后的完整版本
import numpy as np
import random
...
def simulate_robot(gene, gui=False, show_goal=True, sim_time=8.0):
    ...
    if num_links == 0:
        ...
        linkParentIndices = [i for i in range(num_links)]
        linkJointTypes = []
        linkJointAxis = []
        ...
        print(f"[DEBUG] num_links_total = {num_links_total}, num_links = {num_links}")
        print(f"[DEBUG] shapes length = {len(gene['shapes'])}")
        print(f"[DEBUG] joint_types length = {len(gene['joint_types'])}")
