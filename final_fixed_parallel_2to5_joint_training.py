...
2. 修复目标位置问题 - 参考标准MuJoCo Reacher，目标会在物理模拟中微调位置
3. 所有环境的目标行为现在与标准2关节Reacher一致
"""
...
# 修复的XML配置 - 完全参考标准MuJoCo Reacher的目标设置
def get_final_fixed_3joint_xml():
    """获取最终修复的3关节XML - 目标行为与标准Reacher一致"""
    return """
<mujoco model="final_fixed_3joint">
  ...
  <worldbody>
    ...
    <!-- 目标 - 完全参考标准MuJoCo Reacher，使用相同的slide关节参数 -->
...
def get_final_fixed_4joint_xml():
    """获取最终修复的4关节XML"""
    return """
<mujoco model="final_fixed_4joint">
  <compiler angle="radian" inertiafromgeom="true"/>
  <default>
    <joint armature="1" damping="1" limited="true"/>
    <geom contype="0" friction="1 0.1 0.1" rgba="0.7 0.7 0 1"/>
...
def get_final_fixed_5joint_xml():
    """获取最终修复的5关节XML"""
    return """
<mujoco model="final_fixed_5joint">
  <compiler angle="radian" inertiafromgeom="true"/>
  <default>
    <joint armature="1" damping="1" limited="true"/>
    <geom contype="0" friction="1 0.1 0.1" rgba="0.7 0.7 0 1"/>
...
# 最终修复的环境类 - 目标行为与标准Reacher一致
class FinalFixed3JointReacherEnv(MujocoEnv):
    """最终修复的3关节Reacher环境 - 目标行为与标准Reacher一致"""
    
    def __init__(self, render_mode=None, **kwargs):
        print("🌟 FinalFixed3JointReacherEnv 初始化")
        
        self.xml_file = tempfile.NamedTemporaryFile(mode='w', suffix='.xml', delete=False)
        self.xml_file.write(get_final_fixed_3joint_xml())
        self.xml_file.flush()
        
        print(f"   XML文件: {self.xml_file.name}")
        print(f"   🔧 修复: 目标行为与标准2关节Reacher完全一致")
        print(f"   🎯 目标会在物理模拟中微调位置，就像标准Reacher一样")
        ...
        print("✅ FinalFixed3JointReacherEnv 创建完成")
        print(f"   观察空间: {self.observation_space}")
        print(f"   动作空间: {self.action_space}")
        print(f"   关节数量: {self.model.nq} (包含目标slide关节)")
    ...
    def _get_obs(self):
        ...
        # 注意：qpos包含5个元素：3个关节角度 + 2个目标位置（target_x, target_y）
        theta = self.data.qpos.flat[:3]  # 只取前3个关节角度
        obs = np.concatenate([
            np.cos(theta),                    # 3个cos值
            np.sin(theta),                    # 3个sin值
    ...
    def reset_model(self):
        ...
        # 重要：目标位置的随机化范围要与XML中的slide关节范围一致
        # 3关节的可达范围约为0.3，所以目标范围设为±0.45
        qpos[-2:] = self.np_random.uniform(low=-0.45, high=0.45, size=2)
        
        self.set_state(qpos, qvel)
        self.step_count = 0
        return self._get_obs()

class FinalFixed4JointReacherEnv(MujocoEnv):
    """最终修复的4关节Reacher环境"""
    
    def __init__(self, render_mode=None, **kwargs):
        print("🌟 FinalFixed4JointReacherEnv 初始化")
        
        self.xml_file = tempfile.NamedTemporaryFile(mode='w', suffix='.xml', delete=False)
        self.xml_file.write(get_final_fixed_4joint_xml())
        self.xml_file.flush()
        
        print(f"   XML文件: {self.xml_file.name}")
        print(f"   🔧 修复: 目标行为与标准Reacher一致")
        ...
        print("✅ FinalFixed4JointReacherEnv 创建完成")
    ...
    def _get_obs(self):
        theta = self.data.qpos.flat[:4]  # 4个关节角度
        obs = np.concatenate([
            np.cos(theta),
            np.sin(theta),
    ...
    def reset_model(self):
        ...
        # 4关节的可达范围约为0.32，目标范围设为±0.55
        qpos[-2:] = self.np_random.uniform(low=-0.55, high=0.55, size=2)
        
        self.set_state(qpos, qvel)
        self.step_count = 0
        return self._get_obs()

class FinalFixed5JointReacherEnv(MujocoEnv):
    """最终修复的5关节Reacher环境"""
    
    def __init__(self, render_mode=None, **kwargs):
        print("🌟 FinalFixed5JointReacherEnv 初始化")
        
        self.xml_file = tempfile.NamedTemporaryFile(mode='w', suffix='.xml', delete=False)
        self.xml_file.write(get_final_fixed_5joint_xml())
        self.xml_file.flush()
        ...
        print("✅ FinalFixed5JointReacherEnv 创建完成")
    ...
    def _get_obs(self):
        theta = self.data.qpos.flat[:5]  # 5个关节角度
        obs = np.concatenate([
            np.cos(theta),
            np.sin(theta),
    ...
    def reset_model(self):
        ...
        # 5关节的可达范围约为0.3，目标范围设为±0.65
        qpos[-2:] = self.np_random.uniform(low=-0.65, high=0.65, size=2)
...
def make_final_3joint_env():
    """创建最终修复的3关节环境工厂函数"""
    def _init():
        env = FinalFixed3JointReacherEnv(render_mode='human')
        env = MixedJointObservationWrapper(env, target_obs_dim=19)
...
def make_final_4joint_env():
    """创建最终修复的4关节环境工厂函数"""
    def _init():
        env = FinalFixed4JointReacherEnv(render_mode='human')
        env = MixedJointObservationWrapper(env, target_obs_dim=19)
...
def make_final_5joint_env():
    """创建最终修复的5关节环境工厂函数"""
    def _init():
        env = FinalFixed5JointReacherEnv(render_mode='human')
        env = MixedJointObservationWrapper(env, target_obs_dim=19)
...
def train_final_fixed_parallel_2to5_joint(total_timesteps: int = 40000):
    """
    最终修复的2-5关节并行训练
    1. 使用SubprocVecEnv实现真正的多进程并行渲染
    2. 修复目标位置问题 - 所有环境的目标行为现在与标准2关节Reacher一致
    """
    print("🚀 最终修复的2-5关节并行Reacher训练")
    print(f"🎯 同时训练{SUPPORTED_JOINTS}关节Reacher")
    print("💡 修复1: 真正的多进程并行渲染")
    print("💡 修复2: 所有环境的目标行为与标准2关节Reacher完全一致")
    print("💡 修复3: 目标会在物理模拟中微调位置，就像标准Reacher一样")
    print("="*60)
    ...
    env_fns = []
    env_makers = [make_final_2joint_env, make_final_3joint_env, make_final_4joint_env, make_final_5joint_env]
    
    for i, (joints, maker) in enumerate(zip(SUPPORTED_JOINTS, env_makers)):
        env_fns.append(maker())
        print(f"   ✅ {joints}关节环境已添加 (目标行为已修复)")
    ...
    print(f"   🎯 所有环境的目标现在都会像标准Reacher一样微调位置")
    ...
    model = SAC(
        ...
        learning_starts=800,
        device='cpu',
        tensorboard_log="./tensorboard_logs/final_fixed_parallel_2to5/",
        batch_size=256,
        buffer_size=100000,
        learning_rate=3e-4,
        gamma=0.99,
    ...
    # 开始训练
    print(f"\\n🎯 开始最终修复的2-5关节训练 ({total_timesteps}步)...")
    print("💡 您现在应该看到4个MuJoCo窗口同时训练:")
    print("   🔸 窗口1: 2关节Reacher (标准)")
    print("   🔸 窗口2: 3关节Reacher (目标行为已修复)")
    print("   🔸 窗口3: 4关节Reacher (目标行为已修复)")
    print("   🔸 窗口4: 5关节Reacher (目标行为已修复)")
    print("💡 所有环境共享同一个神经网络模型")
    print("💡 所有环境的目标球现在都会像标准Reacher一样微调位置")
    ...
    try:
        ...
        # 保存最终模型
        model.save("models/final_fixed_parallel_2to5_joint_final")
        print("💾 最终修复模型已保存: models/final_fixed_parallel_2to5_joint_final")
        ...
        model.save("models/final_fixed_parallel_2to5_joint_interrupted")
        print("💾 中断模型已保存")
        return model
...
def main():
    """主函数"""
    print("🌟 最终修复的2-5关节Reacher训练系统")
    print("🎯 修复1: 真正的多进程并行渲染")
    print("🎯 修复2: 所有环境的目标行为与标准2关节Reacher完全一致")
    print("💡 现在所有环境的目标都会像标准Reacher一样微调位置")
    print()
    
    try:
        # 开始最终修复的2-5关节训练
        train_final_fixed_parallel_2to5_joint(total_timesteps=40000)
        
        print(f"\\n🎉 最终修复的2-5关节训练完成！")
        print(f"💡 您应该看到了4个环境同时训练的效果")
        print(f"✅ 所有环境的目标球现在都会像标准Reacher一样微调位置")
