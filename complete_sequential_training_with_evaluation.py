...
class SequentialReacherEnv(MujocoEnv):
    ...
    def __init__(self, xml_content, num_joints, link_lengths, render_mode=None, **kwargs):
        ...
        print(f"✅ {num_joints}关节Reacher创建完成 ({reward_type})")
...
def train_single_joint_model(num_joints, total_timesteps=30000):
    ...
    model = SAC(
        ...
        device='cpu',
        tensorboard_log=f"./tensorboard_logs/complete_sequential_{num_joints}joint/",
        batch_size=256,
        buffer_size=100000,
        learning_rate=1e-4,
        gamma=0.99,
    ...
    try:
        ...
        # 保存模型
        model_path = f"models/complete_sequential_{num_joints}joint_reacher"
        ...
        model_path = f"models/complete_sequential_{num_joints}joint_reacher_interrupted"
        model.save(model_path)
...
def main():
    ...
    print("🌟 完整的依次训练和测试2-5关节Reacher系统")
    print("🎯 策略: 每个关节数单独训练，统一使用标准Reacher-v5奖励")
    print("🔧 统一标准Reacher-v5奖励:")
    print(f"   1. 距离奖励: -1.0 * distance_to_target")
    print(f"   2. 控制惩罚: -0.1 * sum(action²)")
    print(f"   3. 成功阈值: 2关节={SUCCESS_THRESHOLD_2JOINT}m, 3+关节={SUCCESS_THRESHOLD_RATIO:.1%}*R")
    print(f"   4. 目标分布统一按R取比例: {TARGET_MIN_RATIO:.1%} ~ {TARGET_MAX_RATIO:.1%} * R")
    print(f"   5. 动力学时间尺度一致: 统一gear=200、density=1000、damping=1")
    ...
    if all_results:
        ...
        for result in all_results:
            print(f"   - models/complete_sequential_{result['num_joints']}joint_reacher.zip")
